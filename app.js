...
const DEFAULT_CATEGORIES = [
  { name: 'AI Breakthroughs', slug: 'ai-breakthroughs' },
  { name: 'Cloud & DevOps', slug: 'cloud-devops' },
  { name: 'Cybersecurity', slug: 'cybersecurity' },
  { name: 'Chips & Compute', slug: 'chips-compute' },
  ...
  { name: 'Policy & Regulation', slug: 'policy-regulation' }
];
...
const CATEGORY_ALIAS_MAP = new Map([
  ...
  ['breakthroughs', 'ai-breakthroughs'],
  ...
  ['chips', 'chips-compute'],
  ['chip', 'chips-compute'],
  ['semiconductors', 'chips-compute'],
  ['compute', 'chips-compute'],
  ['chips-and-compute', 'chips-compute'],
  ...
  ['autonomy', 'robotics-automation'],
  ['policy', 'policy-regulation'],
  ['regulation', 'policy-regulation'],
  ['ai-policy', 'policy-regulation'],
...
app.locals.siteName = 'techiaid.com';
app.locals.language = 'en';

const rawSiteBaseUrl = process.env.SITE_BASE_URL || 'https://techiaid.com';
...
const SITE_DESCRIPTION =
  ...
  'Techi Aid rewrites the biggest stories across AI, cloud infrastructure, chips, consumer hardware, cybersecurity, and policy into fast, verifiable briefings.';
...
const CONTACT_EMAIL = process.env.CONTACT_EMAIL || 'hello@techiaid.com';
const CONTACT_TWITTER =
  process.env.CONTACT_TWITTER || 'https://twitter.com/techiaid';
const CONTACT_LINKEDIN =
  process.env.CONTACT_LINKEDIN || 'https://www.linkedin.com/company/techiaid';
...
const SYNONYM_REPLACEMENTS = new Map([
  ...
  ['announcement', ['reveal', 'launch note', 'disclosure']],
  ['company', ['firm', 'organization', 'vendor']],
  ['companies', ['firms', 'organizations', 'vendors']],
  ['startup', ['young company', 'venture', 'emerging firm']],
  ['product', ['platform', 'service', 'solution']],
  ['update', ['revision', 'patch', 'refresh']],
  ['updated', ['revised', 'refreshed', 'patched']],
  ...
  ['feature', ['capability', 'function', 'module']],
  ['features', ['capabilities', 'functions', 'modules']],
  ['event', ['showcase', 'presentation', 'briefing']],
  ['user', ['customer', 'operator', 'team']],
  ['users', ['customers', 'operators', 'teams']],
  ['engineer', ['developer', 'builder', 'technologist']],
  ['engineers', ['developers', 'builders', 'technologists']],
  ['researcher', ['scientist', 'analyst', 'investigator']],
  ['researchers', ['scientists', 'analysts', 'investigators']],
  ['model', ['system', 'architecture', 'stack']],
  ['models', ['systems', 'architectures', 'stacks']],
  ['data', ['datasets', 'signals', 'inputs']],
  ['hardware', ['devices', 'equipment', 'gear']],
  ['software', ['applications', 'programs', 'tools']],
  ['platform', ['ecosystem', 'service', 'system']],
  ['platforms', ['ecosystems', 'services', 'systems']],
  ['community', ['network', 'audience', 'ecosystem']],
  ...
  ['popular', ['widely used', 'in demand', 'frequently deployed']]
]);
...
const INTRO_PHRASES = [
  'Analysts following the sector note that ',
  'In the latest AI development, ',
  'Industry sources point out that ',
  'According to the newest filings, ',
  'Market watchers highlight that '
];
...
const generateUniqueTitleLocally = (title, referenceParagraphs) => {
  ...
  if (isTitleSimilar(title, candidate)) {
    ...
    if (!candidate) {
      candidate = `${prefix} ${capitalizeSentence('Latest AI development')}`;
    }
  }
...
const buildArticleImageSvg = (article) => {
  ...
  const siteLabel = escapeForXml(app.locals.siteName || 'techiaid.com');
...
const buildRssFeedXml = (articles) => {
  const siteName = app.locals.siteName || 'techiaid.com';
    ...
    const headers = {
      ...
      ...(isOpenRouter
        ? {
            ...
            'X-Title': process.env.OPENROUTER_SITE_TITLE || 'techiaid.com'
          }
        : {
            'Accept-Language': 'en'
          })
...
const normalizeCategoryEntry = (category) => {
  ...
  return DEFAULT_CATEGORY_LOOKUP.get('consumer-tech') || null;
};
...
const deriveCategoriesFromKeywords = (article) => {
  ...
  if (detected.length === 0) {
    const fallback = DEFAULT_CATEGORY_LOOKUP.get('consumer-tech');
    if (fallback) {
...
const buildCategoryMap = (articles) => {
  ...
  articles.forEach((article) => {
    ...
    if (
      ...
      normalizedCategories.every((category) => category.slug === 'consumer-tech')
    ) {
...
app.listen(PORT, () => {
  console.log(`techiaid.com stage server running at http://localhost:${PORT}`);
});
